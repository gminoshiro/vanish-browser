...
### 検証コード・失敗した実装の削除 ⚠️ 重要
**原則**: 検証で失敗したコード、使われていないコードは即座に削除

**なぜ重要か**:
- 残すとバグの元になる（例: BUG-045 オーバーレイDLボタン）
- コードが肥大化して保守性が下がる
- 他の開発者（未来の自分）が混乱する

**削除対象**:
1. **失敗した実装**: 動作しない、採用されなかった機能
2. **検証用コード**: テスト目的で追加したが不要になったコード
3. **コメントアウトコード**: 「後で使うかも」は禁止
4. **未使用の変数・関数**: 呼び出されていないもの
5. **未使用のインポート**: 使われていないライブラリ

**削除フロー**:
1. 検証失敗が確定したら即座に削除
2. 削除内容をドキュメントに記録（なぜ削除したか）
3. 必要なら履歴はgit logで確認可能
4. 「念のため残す」は禁止

**例外**:
- ドキュメント（検証記録）は残す（例: FEATURE-011-FINAL-SUMMARY.md）
- git履歴は残る（必要なら復元可能）
